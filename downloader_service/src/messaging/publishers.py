"""
messaging/publishers.py

Functions for publishing messages to RabbitMQ queues and handling URL publishing logic.
"""
...
async def publish_embeddings(image_id: int, image_url: str, image_path: str):
    """
    Publish a message to the embedding queue for processing embeddings of the downloaded image.
    """
    message = {
        "image_id": image_id,
        "image_url": image_url,
...
async def process_and_publish_chunk(urls: List[str]):
    """
    Filter URLs that haven't been processed and publish them to the download queue.
    """
